...
from functions import detectBox, OutOfRangeWarning, DistanceWarning, StraightWarning, find_skinColor_crcb, crcb_oval,\
    get_rightEarPoint, get_leftEarPoint, add_white_bg, beauty_face, aug, cutEdge
...
sizeIndex = 0
global capture, segmentation_flag, beauty_face_flag, light_enhance_flag
capture = False
    ...
    while True:
        ...
        if not success:
            ...
            if segmentation_flag:
                display_frame = add_white_bg(display_frame)

            if capture:
                capture = False
                snap = cutEdge(box_upperLeft_col, box_h, box_w, original_frame)
                now = datetime.datetime.now()
                save_path = os.path.sep.join(['shots', "shot_{}.png".format(str(now).replace(":", ''))])
                cv2.imwrite(save_path, snap)
...
def tasks():
    if request.method == 'POST':
        if request.form.get('click') == 'Capture':
            global capture
            capture = True
        elif request.form.get('segment') == 'Segment':
            global segmentation_flag
            segmentation_flag = not segmentation_flag
        elif request.form.get('beauty') == 'Beauty':
            global beauty_face_flag
            beauty_face_flag = not beauty_face_flag
        elif request.form.get('light') == 'Light':
            global light_enhance_flag
